        ...
        <tbody>
          @forelse ($productos as $producto)
            <tr>
              ...
              <td>
                <a href="{{ route('productos.show', $producto) }}" class="link-bronze action-link">Ver</a>
                <a href="{{ route('productos.edit', $producto) }}" class="link-bronze action-link">Editar</a>
                <form action="{{ route('productos.destroy', $producto) }}" method="POST" style="display:inline;">
                  @csrf
                  @method('DELETE')
                  <button type="submit" class="action-btn" onclick="return confirm('¿Seguro que deseas eliminar este producto?')">Eliminar</button>
                </form>
              </td>
            </tr>
          @empty
